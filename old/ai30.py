...
DEFAULT_INITIAL_PROMPT = (
    "Let's have a discussion about world civilization. "
 #   "Let's talk about how to make my girlfriend laugh. "
 #   " let's talk about how to have a good time at work flipping burgers. "
...
FILE_PATH = "committee_notes.txt"
WRITE_THRESHOLD = 0.8  # Agreement threshold for writing to file (0.0 to 1.0)
...
def write_to_file(text: str):
    """Writes text to the file, appending to existing content."""
    try:
        with open(FILE_PATH, "a", encoding="utf-8") as f:
            f.write(text + "\n")
        colored_print(Colors.SUCCESS, f"Successfully wrote to {FILE_PATH}")
    except Exception as e:
        colored_print(Colors.ERROR, f"Error writing to {FILE_PATH}: {e}")

# --- Agreement Check Function ---

def check_agreement(response_a: str, response_b: str) -> bool:
    """
    Checks for agreement between two responses.  This is a placeholder.
    You'll need to replace this with a more sophisticated method
    (e.g., semantic similarity, keyword overlap, etc.).
    """
    # Simple example: Check if responses contain similar keywords
    keywords_a = set(response_a.lower().split())
    keywords_b = set(response_b.lower().split())
    common_keywords = keywords_a.intersection(keywords_b)
    agreement_ratio = len(common_keywords) / max(len(keywords_a), len(keywords_b))
    return agreement_ratio >= WRITE_THRESHOLD
...
def simulate_conversation(
    ...
    try:
        ...
        for turn in range(num_turns):
                ...
                if current_turn < num_turns:
                    ...
                    active_thread = threading.Thread(
                         ...
                         args=(
                             request_session,
                             next_server_info,
                             conversation_history.copy(), # History *without* failed response
                             result_queue,
                             next_server_id,
                             DEFAULT_MAX_TOKENS,
                             DEFAULT_TEMPERATURE,   # <<< Add comma here
                             DEFAULT_STOP_WORDS,    # <<< Use the full variable name
                             REQUEST_TIMEOUT
                         ),
                         daemon=True
                    )
            ...
            # --- Check for Agreement and Write to File ---
            if current_turn < num_turns and pending_server_id:
                # Wait for the other server's response before checking agreement
                try:
                    other_server_id, other_response = result_queue.get(timeout=q_timeout)
                    if other_server_id == pending_server_id:
                        # Now we have both responses
                        if check_agreement(response, other_response):
                            combined_text = f"Turn {current_turn}:\nServer A: {response}\nServer B: {other_response}\n"
                            write_to_file(combined_text)
                        else:
                            colored_print(Colors.SYSTEM, "Responses did not meet the agreement threshold.")
                    else:
                        colored_print(Colors.ERROR, f"Unexpected response ID from queue: {other_server_id}")
                except Empty:
                    colored_print(Colors.ERROR, f"Timeout waiting for other server's response for agreement check.")
